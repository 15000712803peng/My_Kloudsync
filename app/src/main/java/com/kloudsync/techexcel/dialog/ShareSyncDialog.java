...
public class ShareSyncDialog implements View.OnClickListener, HeaderRecyclerAdapter.OnItemClickListener, IRongCallback.ISendMessageCallback {
    ...
    public ShareSyncDialog(Context context) {
        mContext = context;
        initDialog();
        getFriendList();
    }

    public SoundtrackBean getSync() {
        return sync;
    }

    public void setSync(SoundtrackBean sync) {
        this.sync = sync;
    }

    public void setDocument(LineItem document) {
    ...
    private void initSendView(MyFriend friend) {
        ...
        if (!TextUtils.isEmpty(sync.getAvatarUrl())) {
            syncImage.setImageURI(Uri.parse(sync.getAvatarUrl()));
        }
        if (!TextUtils.isEmpty(sync.getUserName())) {
            syncName.setText(sync.getUserName());
        }
        if (!TextUtils.isEmpty(sync.getDuration())) {
            syncDuration.setText(sync.getDuration());
        }
