 ...
 */
public class BaseCallActivity2 extends Activity implements  PickupDetector.PickupDetectListener {
    ...
    public void onOutgoingCallRinging() {
        mMediaPlayer = MediaPlayer.create(this, io.rong.callkit.R.raw.voip_outgoing_ring);
    ...
    protected void onPause() {
        ...
        if (shouldShowFloat) {
            ...
            if (action != null) {
                ...
                showOnGoingNotification(getString(io.rong.callkit.R.string.rc_call_on_going),
                        mediaType == RongCallCommon.CallMediaType.AUDIO.getValue() ? getString(io.rong.callkit.R.string.rc_audio_call_on_going) : getString(io.rong.callkit.R.string.rc_video_call_on_going));
            }
        }
    }
    ...
    void onMinimizeClick(View view) {
        if (Build.BRAND.toLowerCase().contains("xiaomi")) {
            if (PermissionCheckUtil.canDrawOverlays(this)) {
                ...
                Toast.makeText(this, io.rong.callkit.R.string.rc_voip_float_window_not_allowed, Toast.LENGTH_LONG).show();
            }
        } else {
            finish();
        }
