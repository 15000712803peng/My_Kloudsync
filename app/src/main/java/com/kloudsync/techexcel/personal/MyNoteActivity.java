...
public class MyNoteActivity extends BaseActivity<MyNotePresenter> implements MyNoteView {
	...
	protected void initView() {
		EverPenManger.getInstance(this).addListener(mPresenter);
		...
		mLiveNote.setCreatedDate("");
		mLiveNote.setNoteType(TYPE_LIVE_NOTE);
		notes.add(mLiveNote);
		boolean isNotchScreen = PenDotTool.hasNotchScreen(this);
		DisplayMetrics dm = new DisplayMetrics();
		getWindowManager().getDefaultDisplay().getMetrics(dm);
		mMyNotesAdapter = new MyNotesAdapter(this, notes, isNotchScreen, dm.widthPixels, dm.heightPixels);
	...
	protected void onDestroy() {
		...
		EverPenManger.getInstance(this).removeListener(mPresenter);
		super.onDestroy();
	}
}
