...
//                                MeetingKit.getInstance().startMeeting();
                                ...
                                MeetingKit.getInstance().prepareJoin(NoteViewActivityV2.this, meetingConfig);
        ...
        meetingWarningDialog.setOnUserOptionsListener(new MeetingWarningDialog.OnUserOptionsListener() {
            ...
            public void onUserStart() {
                if (type == 1) {
                    ...
                    meetingKit.prepareStart(NoteViewActivityV2.this, meetingConfig, meetingConfig.getLessionId() + "");
                } else {
                    MeetingKit.getInstance().prepareJoin(NoteViewActivityV2.this, meetingConfig);
                }
            }
...
//        }
    ...
    private MeetingConfig getConfig() {
        ...
        meetingConfig.setNoteAttachmentUrl(data.getStringExtra("url"));
        meetingConfig.setLocalFileId(data.getStringExtra("local_file_id"));
        meetingConfig.setCurrentNoteId(data.getIntExtra("note_id", 0));
...
//        EventBus.getDefault().post(refreshMembers);
    ...
    public synchronized void receiveDocuments(EventMeetingDocuments documents) {
        ...
        if (this.documents != null && this.documents.size() > 0) {
            ...
            if(!TextUtils.isEmpty(meetingConfig.getLocalFileId())){
                if(meetingConfig.getDocumentId().equals(meetingConfig.getDocument().getItemID() + "")){
                    ...
                    Log.e("receiverDocuemnts", "is_note");
                    meetingConfig.getDocument().setLocalFileID(meetingConfig.getLocalFileId());
                    meetingConfig.getDocument().setNoteID(meetingConfig.getCurrentNoteId());
                }
    ...
    public synchronized void receiveSocketMessage(EventSocketMessage socketMessage) {
        ...
        switch (action) {
            ...
            case SocketMessageManager.MESSAGE_NOTE_DATA:  // 浮窗或主界面正在展示的场景下
                Log.e("check_document","document:" + meetingConfig.getDocument());
                if(meetingConfig.getDocument() != null && (meetingConfig.getDocument().getItemID() + "").equals(meetingConfig.getDocumentId())){
                    if (socketMessage.getData().has("retData")) {
                        try {
                            ...
                            if (currentNoteId != noteId) {
                                ...
                                newNoteDatas.clear();
                                NoteViewActivityV2.TempNoteData _noteData = new NoteViewActivityV2.TempNoteData();
...
//                noteWeb.load("javascript:whiteboard");
    ...
    private void doDrawDTNewBorder(int x, int y, int width, int height, JSONObject info) throws JSONException {
        ...
        if(noteWeb.getVisibility() == View.VISIBLE){
...
//        if (!TextUtils.isEmpty(meetingConfig.getLocalFileId()) && meetingConfig.getLocalFileId().contains(".")) {
//            Observable.just("load_note_page").observeOn(AndroidSchedulers.mainThread()).subscribe(new Consumer<String>() {
//                @Override
//                public void accept(String s) throws Exception {
//                    hideEnterLoading();
...
//        }
    ...
    public void afterChangePageFunction(final int pageNum, int type) {
        ...
        if (meetingConfig.getDocument() != null && (meetingConfig.getDocument().getItemID() + "").equals(meetingConfig.getDocumentId())) {
            handleBluetoothNote(meetingConfig.getDocument().getAttachmentUrl());
        }
    }
    ...
    public synchronized void callAppFunction(final String action, final String data) {
        ...
        Observable.just(data).observeOn(AndroidSchedulers.mainThread()).doOnNext(new Consumer<String>() {
            @Override
            public void accept(String s) throws Exception {
                PageActionsAndNotesMgr.handleNoteActions(NoteViewActivityV2.this, action, new JSONObject(data), meetingConfig);
            }
        }).subscribe();
            ...
            public void uploadError(String message) {
                ...
                if (bottomFilePop != null && bottomFilePop.isShowing()) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            new CenterToast.Builder(NoteViewActivityV2.this).
                ...
                if (response != null && response.isSuccessful() && response.body() != null) {
                    if (response.body().getData() != null) {
                        ...
                        if (devices != null && devices.size() > 0) {
                            ...
                            devicesListDialog.getPopwindow(NoteViewActivityV2.this);
    ...
    private void gotoScanTv() {
        ...
        if (meetingConfig.getType() == MeetingType.MEETING) {
            ...
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    Intent intent = new Intent(NoteViewActivityV2.this, MipcaActivityCapture.class);
            ...
            public void accept(JSONObject jsonObject) throws Exception {
                if (jsonObject.has("RetCode")) {
                    if (jsonObject.getInt("RetCode") == 0) {
                        new CenterToast.Builder(NoteViewActivityV2.this).setSuccess(true).setMessage(getString(R.string.operate_success)).create().show();
                    } else {
                        new CenterToast.Builder(NoteViewActivityV2.this).setSuccess(false).setMessage(getString(R.string.operate_failure)).create().show();
...
//    }
    ...
    private void showCreateSyncDialog() {
        ...
        yinxiangCreatePopup.getPopwindow(NoteViewActivityV2.this);
        ...
        if (TextUtils.isEmpty(noteId)) {
            ...
            if (!QueryLocalNoteTool.noteIsExist(this, noteId)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(NoteViewActivityV2.this, "笔记在本地设备不存在", Toast.LENGTH_SHORT).show();
                    }
                });
                return;
            }
...
//                context.startForegroundService(service);
    ...
    private void handleBluetoothNote(final String url) {
        ...
        Observable.just(url).observeOn(Schedulers.io()).map(new Function<String, String>() {
            ...
            public void accept(JSONObject jsonObject) throws Exception {
                ...
                Log.e("ShowDotPanData", "ShowDotPanData,noteweb:" + (noteWeb.getVisibility() == View.VISIBLE));
                if(noteLayout.getVisibility() == View.VISIBLE){
                    noteWeb.load("javascript:FromApp('" + key + "'," + _data + ")", null);
                }else {
            ...
            public void accept(JSONObject jsonObject) throws Exception {
                if (!newNoteDatas.isEmpty()) {
                    Observable.fromIterable(newNoteDatas).observeOn(AndroidSchedulers.mainThread()).doOnNext(new Consumer<NoteViewActivityV2.TempNoteData>() {
                        @Override
                        public void accept(NoteViewActivityV2.TempNoteData tempNoteData) throws Exception {
    ...
    public void handleBluetoothNote(EventOpenOrCloseBluethoothNote note) {
        if (note != null) {
            if(meetingConfig.getDocument() != null && TextUtils.isEmpty(meetingConfig.getDocument().getLocalFileID())){
                if (note.getStatus() == 0) {
                    finish();
                    return;
                }
