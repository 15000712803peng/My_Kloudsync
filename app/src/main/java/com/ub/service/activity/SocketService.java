...
//        registerNumberReceiver();
    ...
    private void initReconnectTimeTask() {
        if (mReconnectTimer == null && mReconnectTimerTask == null) {
            mReconnectTimer = new Timer();
            mReconnectTimerTask = new TimerTask() {
                @Override
                public void run() {
                    if (mStopTimerTask) return;
                    mCount++;
                    if (mCount >= 10) {//如果10秒内没有收到socket消息则重新初始化连接WebSocket
            ...
            mStopTimerTask = false;
            mReconnectTimer.schedule(mReconnectTimerTask, 0, 1000);
        }
    }
