...
//                        activity.promoteAuditor((String) (msg.obj));
                        break;
                    case 0x1205:
                        activity.currentShowPdf = (LineItem) msg.obj;
                        activity.changedocumentlabel(activity.currentShowPdf);
                        break;
                    case 0x4010:
...
//                                openVideoByViewType();
    ...
    private void initView() {
        ...
        if (isFinishCourse) {
            playagorarl = (RelativeLayout) findViewById(R.id.playagorarl);
            playagora = (ImageView) findViewById(R.id.playagora);
            ...
            playagora.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    firstPlayAgora();
                }
    ...
    private void firstPlayAgora() {
        playagorarl.setVisibility(View.GONE);
        String url = AppConfig.URL_PUBLIC + "LessonSoundtrack/List?lessonID=" + (isFinishCourse ? fileMeetingId : meetingId) + "&attachmentID=" + currentAttachmentId;
        ServiceInterfaceTools.getinstance().getSoundList(url, ServiceInterfaceTools.GETSOUNDLIST,
                new ServiceInterfaceListener() {
                    @Override
                    public void getServiceReturnData(Object object) {
                        List<SoundtrackBean> list = (List<SoundtrackBean>) object;
                        if (list != null && list.size() > 0) {
                            SoundtrackBean soundtrackBean2 = list.get(0);
                            ServiceInterfaceTools.getinstance().getSoundItem(AppConfig.URL_PUBLIC + "Soundtrack/Item?soundtrackID=" + soundtrackBean2.getSoundtrackID(),
...
//                        downEveryOnePdf(url, currentpageNum);
    ...
    public void afterChangePageFunction(final String pageNum, int type) {
        ...
        ServiceInterfaceTools.getinstance().getNoteListV2(AppConfig.URL_PUBLIC + "DocumentNote/List?syncRoomID=" + meetingId + "&documentItemID=" + currentAttachmentId + "&pageNumber=" + currentAttachmentPage + "&userID=" + AppConfig.UserID, ServiceInterfaceTools.GETNOTELISTV2, new ServiceInterfaceListener() {
            @Override
            public void getServiceReturnData(Object object) {
...
//                    {"type":38,"LinkID":123,"LinkProperty":{"X":0.2683315621679065,"Y":0.37898089171974525}}
                ...
                if (!TextUtils.isEmpty(selectCusterId)) {
                    ServiceInterfaceTools.getinstance().getNoteListV3(AppConfig.URL_PUBLIC + "DocumentNote/List?syncRoomID=" + meetingId + "&documentItemID=" + currentAttachmentId + "&pageNumber=" + currentAttachmentPage + "&userID=" + selectCusterId, ServiceInterfaceTools.GETNOTELISTV3, new ServiceInterfaceListener() {
                        @Override
                        public void getServiceReturnData(Object object) {
                            List<NoteDetail> noteDetails = (List<NoteDetail>) object;
                            if (noteDetails != null && noteDetails.size() > 0) {
    ...
    public void autoChangeFileFunction(int diff) {
        ...
        if (isHavePresenter()) {
            ...
            if (diff == 1) {
                ...
                notifySwitchDocumentSocket(currentShowPdf, "0");
            } else if (diff == -1) {
...
//                                initMute(true);
        ...
        mLeftAgoraAdapter = new LeftAgoraAdapter(WatchCourseActivity2.this);
        ...
        mRightAgoraAdapter = new LeftAgoraAdapter(WatchCourseActivity2.this);
...
//            {"LinkID":0,"LinkProperty":{"X":0.29383634431455896,"Y":0.35509554140127386}}
    ...
    private void openMyNoteBook(final JSONObject linkProperty) {
        ...
        SelectMyNoteDialog selectNoteDialog = new SelectMyNoteDialog(WatchCourseActivity2.this, 0 + "");
